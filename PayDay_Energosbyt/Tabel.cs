...
namespace PayDay_Energosbyt
{
    public partial class Tabel : Form
    {
        ...
        public Tabel(string id, MySqlOperations mySqlOperations, MySqlQueries mySqlQueries)
        {
            InitializeComponent();
            ID = id;
            MySqlOperations = mySqlOperations;
            MySqlQueries = mySqlQueries;
            MySqlOperations.Select_DataGridView(MySqlQueries.Select_Tabel, dataGridView1, ID);
        }
        ...
        private void Update_Table(string Month = null, string Year = null)
        {
            if (Month == null && Year == null)
            {
                if (dateTimePicker2.Checked)
                {
                    ...
                    MySqlOperations.Select_DataGridView(MySqlQueries.Select_Tabel_Filter, dataGridView1, ID, Month, Year);
                }
                else
                {
                    MySqlOperations.Select_DataGridView(MySqlQueries.Select_Tabel, dataGridView1, ID);
        ...
        {
            MySqlOperations.Insert_Update(MySqlQueries.Update_Tabel, ID, comboBox1.Text[0].ToString(), numericUpDown1.Value.ToString(), dataGridView1.SelectedRows[0].Cells[0].Value.ToString());
            Update_Table();
        }
        ...
        private void button1_Click(object sender, EventArgs e)
        {
            try
            {
                MySqlOperations.Insert_Grafik(MySqlQueries ,dateTimePicker1, MySqlQueries.Insert_Tabel, ID);
            }
            catch (Exception ex)
            {
